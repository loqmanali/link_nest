      ...
      children: [
                          ...
                          onPressed: () {
                            if (controller.text.trim().isNotEmpty) {
                              final updatedPost = post.copyWith(
                                highlights: [...post.highlights, controller.text.trim()],
                              );
        ...
        Container(
          ...
          child: post.highlights.isEmpty
              ...
              : Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: post.highlights.asMap().entries.map((entry) {
                    final index = entry.key;
                    final highlight = entry.value;
                    return Container(
                      margin: EdgeInsets.only(
                        bottom: index < post.highlights.length - 1 ? AppTheme.spacing2 : 0,
                      ),
                      ...
                      child: Row(
                        children: [
                          ...
                          if (isEditing.value)
                            GestureDetector(
                              onTap: () {
                                final updatedHighlights = List<String>.from(post.highlights);
                                updatedHighlights.removeAt(index);
                                final updatedPost = post.copyWith(highlights: updatedHighlights);
                                context.read<PostBloc>().add(UpdatePost(updatedPost));
                              },
                              child: const Icon(
                                Icons.close,
